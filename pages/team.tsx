...
function MemberCard(props: MemberCardProps) {
  const {
    name, role, content, avatar, github, index,
  } = props;
  return (
    <Flex
      boxShadow={'lg'}
      ...
      <Flex
        ...
        <chakra.p fontFamily={'Work Sans'} fontWeight={'bold'} fontSize={14} display={'flex'} alignItems={'center'} gap={'5px'}>
...
const Team = () => <>
  ...
  <Flex
    ...
    <SimpleGrid
      ...
      mt={16}
      mx={'auto'}>
      {TEAM_MEMBERS.map((cardInfo, index) => (
        <MemberCard {...cardInfo} index={index} key={index} />
      ))}
    </SimpleGrid>
    <Box>
